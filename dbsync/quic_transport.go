...
func PushSnapshot(ctx context.Context, endpoint string, tlsCfg *tls.Config, meta SnapshotMeta, snapshotPath string) error {
	...
	if err != nil {
		_ = conn.CloseWithError(1, "marshal failed")
	...
	if err != nil {
		_ = conn.CloseWithError(1, "open snapshot failed")
	...
	if meta.Compressed {
		...
		if err := gw.Close(); err != nil {
			_ = conn.CloseWithError(1, "gzip close failed")
		...
		if n != meta.Size {
			_ = conn.CloseWithError(1, "size mismatch")
